...
from bot.helper.index import get_files, related_items_html
...
async def _render_base(page_title, page_content, is_admin=False, extra_head="", selected_label="OTT Lounge"):
    ...
    return (
        ...
        .replace("<!-- SelectedLabel -->", selected_label or "OTT Lounge")
    )
...
async def render_page(
    ...
    if route == "enter":
        ...
        return await _render_base("OTT Lounge: Enter", content, is_admin=False)
    ...
    if route == "home":
        ...
        return await _render_base("OTT Lounge: Home", content, is_admin=is_admin, selected_label="Channel Selector")
    ...
    if route == "playlist":
        ...
        return await _render_base(f"OTT Lounge: {msg}", content, is_admin=is_admin)

    if route == "list":
        content = await _render_template_file("list.html", {"<!-- Theme -->": theme.lower()})
        return await _render_base("OTT Lounge: List", content, is_admin=is_admin)
    ...
    if route == "index":
        ...
        return await _render_base(f"OTT Lounge: {msg}", content, is_admin=is_admin, selected_label=msg)
    ...
    if tag == "video":
        ...
        if duration_sec:
            ...
            duration = "Unknown"
        related_posts = await get_files(chat_id, page=1)
        related_html = related_items_html(related_posts, chat_id, exclude_id=id)
        content = await _render_template_file(
            "video.html",
            {
                "<!-- Title -->": caption,
                ...
                "<!-- RelatedItems -->": related_html,
            },
        )
        extra_head = '<link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/weebzone/weebzone/data/Surf-TG/css/plyr.css">'
        return await _render_base(f"OTT Lounge: {caption}", content, is_admin=is_admin, extra_head=extra_head, selected_label=caption)
    ...
    return await _render_base(f"OTT Lounge: {filename}", content, is_admin=is_admin, selected_label=filename)
